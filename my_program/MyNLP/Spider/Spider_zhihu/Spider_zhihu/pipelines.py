...
class Pipiline_ToCSV(object):
    ...
    def __init__(self):
        #文件的位置
        store_file = os.path.dirname(__file__) + '/spiders/zhihu.csv'
        #打开文件，并设置编码
    ...
    def process_item(self, item, spider):
        line = (item['title'], item['url'], item['source'], item['detail'], item['answer'])
        # 逐行写入
        if item['answer'] != '':
            self.writer.writerow(line)
            self.repeat.write(item['title'] + r'||')
        return item
